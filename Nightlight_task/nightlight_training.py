...
from nightlight_ssl_dataset import NightlightSSLDataset
...
#train_csv = "/cephyr/NOBACKUP/groups/globalpoverty1/JesperBenjamin/imagepath1000.csv"
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ###M
    # Build MS-Resnet18
    # SIGMOID FOR NIGHTLIGHT TASK
    model = MS_NL_ResNet18(num_classes=1, last_layer_activation=torch.nn.Sigmoid())
    ...
    val_indices = np.random.choice(len(train_df), int(val_size*len(train_df)), replace=False)
    df_indices = range(len(train_df))
    train_indices = [x for x in df_indices if x not in val_indices]
    print("Train size: ", len(train_indices))
    print("Val size: ", len(val_indices))
    
    ###M
    train_dataset = NightlightSSLDataset(train_csv, csv_indices=train_indices)
    val_dataset = NightlightSSLDataset(train_csv, csv_indices=val_indices)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            if epoch % args.ckpt_frequency == 0 or epoch == args.epochs - 1:
                ...
                }, is_best=False, filename=os.path.join(args.save_dir, 'checkpoint_nl_lr_{}_epoch_{:04d}.pth.tar'.format(args.lr, epoch)))              
    
        textfile = open(os.path.join(args.save_dir,"losses_nl_lr_{}_epoch_{:04d}_.txt".format(args.lr,epoch)), "w")
...
def train(train_loader, val_loader, model, criterion, optimizer, epoch, args, writer, loss_list, val_loss_list):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            ...
            labels = labels.cuda(args.gpu, non_blocking=True).to(torch.float)
        ...
        #print("outputs size squeezed: ", output.size())
         
        #print("labels ", type(labels), " output: ", type(output))

        # Apply loss fn (BCELoss)
...
def validate(model, loss_fn, val_loader, device):
    ...
    with torch.no_grad():
        for batch_index, (x, y) in enumerate(val_loader, 1):
            inputs, labels = x.cuda(device), y.cuda(device).to(torch.float)
