...
import moco.loader
import moco.builder_geo
import datasets

from data_loader.fmow_dataloader import CustomDatasetFromImages, CustomDatasetFromImagesTemporal
...
train_csv = "../csvs/train_100geoclasses.csv"
temporal = True
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            model.cuda(args.gpu)
            geoclassifier = classifier().cuda(args.gpu)
            ...
            geoclassifier = torch.nn.parallel.DistributedDataParallel(
                geoclassifier, device_ids=[args.gpu])
        else:
            model.cuda()
            geoclassifier = classifier().cuda()
            # DistributedDataParallel will divide and allocate batch_size to all
            # available GPUs if device_ids are not set
            model = torch.nn.parallel.DistributedDataParallel(model)
            geoclassifier = torch.nn.parallel.DistributedDataParallel(geoclassifier)
    ...
    optimizer = torch.optim.SGD(list(model.parameters())+list(geoclassifier.parameters()), args.lr,
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            geoclassifier.load_state_dict(checkpoint['geo_dict'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, geoclassifier, criterion, optimizer, epoch, args, writer)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            if epoch % args.ckpt_frequency == 0 or epoch == args.epochs - 1:
                save_checkpoint({
                    ...
                    'geo_dict': geoclassifier.state_dict(),
...
def train(train_loader, model, geoclassifier, criterion, optimizer, epoch, args, writer):
    ...
    for i, (images, geolabels) in enumerate(train_loader):
        ...
        # compute output
        output, target, embeddings = model(im_q=images[0], im_k=images[1])
        
        preds = geoclassifier.forward(embeddings)
        loss_geocls = CEL(preds, geolabels.cuda()) 
        ...
        acc1_geo, acc5_geo = accuracy(preds, geolabels.cuda(), topk=(1, 5))
